/*function polybius(input, encode) {
    ...
    for (let i = 0; i < input.length; i++){
        let currentCharValue = upperCaseInput.charCodeAt(i)
        if (currentCharValue >= 65 && currentCharValue < 70){
            getCharValues += 1;
        }
        else if (currentCharValue >= 70 && currentCharValue < 76){
            getCharValues += 2;
            }
        else if(currentCharValue >= 75 && currentCharValue <= 80){
            getCharValues += 3;
            }
        else if(currentCharValue >= 80 && currentCharValue < 85){
            getCharValues += 4;  
        }
        else{
            getCharValues += 5;
    ...
    console.log(getCharValues)
    console.log(decoded);
}

polybius("Hamburger". true);*/
